...
$app->post('/users/register', function (Request $request, Response $response, array $args) {
    ...
    try {
        ...
        try {
            ...
            if ($existing_email) {
                $response->getBody()->write(
                    json_encode(array("status" => "fail", "data" => array("Message" => "Email is already taken! Try another one.")))
                );
                return $response;
            }
...
$app->delete("/books/delete", function (Request $request, Response $response, array $args) {
    ...
    try {
        ...
        try {
            ...
            if ($userInfo['token'] !== $jwt) {
                $response->getBody()->write(
                    json_encode(array("status" => "fail", "data" => array("Message" => "Token is invalid or expired.")))
                );
                return $response;
            }
...
$app->post("/authors/add", function (Request $request, Response $response, array $args) {
    ...
    try {
        ...
        try {
            $conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password); // Updated connection string
            $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
